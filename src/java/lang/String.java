 ...
 * --- String 类表示字符串。
 *
 * <p> All string literals in Java programs, such as "abc", are implemented as instances of this class.
 * --- Java程序中的所有字符串，比如“abc”，都是实现了这个类的实例。
 *
 * <p> Strings are constant; their values cannot be changed after they are created.
 * --- 字符串是常量的，其值在创建后就是不可改变的了。
 *
 * <p> String buffers support mutable strings.
 * --- String buffers 支持可变字符串。
 *
 * <p> Because String objects are immutable they can be shared.
 * --- 因为字符串对象是不可变的，所以字符串对象可以被共享。
 * For example:
 * ```
 * String str = "abc";
 * ```
 ...
 * ```
 *
 * <p> The class String includes methods for examining individual characters of the sequence, for comparing strings,
 ...
 * --- String 类包含了很多方法，如检查序列的单个字符、字符串比较、字符串搜索、截取字符串和创建一个字符串并将字符串全部转为大写或小写等。
 *
 * <p> Case mapping is based on the Unicode Standard version specified by the Character class.
 * --- 大小写映射基于由Character类指定的Unicode Standard版本。
 *
 * <p> The Java language provides special support for the string concatenation operator,
 * and for conversion of other objects to strings.
 * --- Java语言对字符串提供了一些专用的方法，如对字符串连接操作符以及将其他对象转换为字符串的一些方法。
 *
 * <p> String concatenation is implemented through the StringBuilder(or StringBuffer) class and its append method.
 * --- String 的连接是通过 StringBuilder 或 StringBuffer 类的 append 方法实现的。
 *
 * <p> String conversions are implemented through the method toString,
 * defined by Object and inherited by all classes in Java.
 * --- 字符串转换通过方法toString实现，该方法由Object定义，并由Java中的所有类继承。
 *
 * <p> For additional information on string concatenation and conversion,
 * see Gosling, Joy, and Steele,The Java Language Specification.
 * --- 有关字符串连接和转换的更多信息，请参见 Gosling、Joy和Steele的书《Java语言规范》。
 *
 * <p> Unless otherwise noted, passing a <tt>null</tt> argument to a constructor or
 * method in this class will cause a {@link NullPointerException} to be thrown.
 * --- 除非有额外说明，给构造器或方法传入一个null值，会产生一个空指针异常被抛出。
 *
 * <p> A String represents a string in the UTF-16 format in which supplementary characters are represented by surrogate pairs
 * (see the section <a href="Character.html#unicode">Unicode Character Representations</a> in the Character class for more information).
 * --- 每个字符是用 UTF-16 编码表示的，其中扩展的字符是用一对即两个 char 表示的。
 *
 * <p> Index values refer to char code units, so a supplementary character uses two positions in a String.
 * --- 索引值引用字符的编码单元，所以拓展的字符在占用两个字节。
 *
 * <p> The String class provides methods for dealing with Unicode code points (i.e., characters),
 * in addition to those for dealing with Unicode code units (i.e., char values).
 * --- String 类提供了一些方法，如对 Unicode 代码点即字符处理，以及 Unicode 代码单元即字符串的处理。
 *
 * @author Lee Boynton
